<template>
  <div>
    <client-only>
      ...
      <script async src="https://cdn.sweettooth.io/assets/storefront.js" charset="utf-8" />
    </client-only>
  </div>
</template>
...
export default {
  ...
  mounted () {
    ...
    const smileData = smileCookie.length > 0 ? JSON.parse(smileCookie) : undefined

    if (
      smileData &&
      smileData.digest &&
      smileData.digest.length > 0
    ) {
